...
from _webmart_api.baseschema import CustomFilterSchema
...
ProductOut = generate_schema(
    ...
    custom=[("category", List[CategorySimpleOut], None)],
)

FullProductOut = generate_schema(
    Product, "FullProductOut", custom=[("category", List[CategorySimpleOut], None)]
)
